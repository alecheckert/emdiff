...
distinct states and the diffusion coefficient corresponding
to each state.
...
There are actually two commands in `emdiff`:
 ...
 - `vbdiff` is a variational Bayes treatment of the same problem.
    ...
    occupancies and diffusion coefficients, given a set of 
    trajectories.
...
```
    ...
    occs_mean, diff_coefs_mean = vbdiff(
        tracks,
        n_states=2,
        pos_cols=["y", "x"],     # columns in *tracks* with 
                                 # the spatial coordinates
...
Here, `occs_mean` are the mean occupancies of each state under the posterior
model and `diff_coefs_mean` are the mean diffusion coefficients of each 
state under the posterior model. The states are always ordered in terms
...
As you can see, we get several additional outputs. What do these mean?
 ...
 - `elbo` is the evidence lower bound for this choice of `n_states`. 
